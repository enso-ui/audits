...
import Audit from '../src/bulma/pages/audit/components/Audit.vue';
...
describe('Index', () => {
    it('includes action, model and id filters plus enum rendering', () => {
        const source = fs.readFileSync(indexPath, 'utf8');

        expect(source).toContain('filters.audits.event');
        expect(source).toContain('filters.audits.auditable_type');
        expect(source).toContain('filters.audits.auditable_id');
        expect(source).toContain('source="system.audit.models"');
